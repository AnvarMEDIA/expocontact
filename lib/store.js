...
/**
 * How long Vercel Blob is told to cache a document at the edge. 60 is the
 * lowest value the API accepts; anything smaller is raised to it silently.
 */
...
export async function writeDoc(name, value) {
  if (hasBlobStore()) {
    ...
    await put(blobPath(name), JSON.stringify(value, null, 2), {
      ...
      allowOverwrite:  true,
      // Blob refuses anything under a minute, so this is the floor. The public
      // URL is served through a CDN that ignores query strings, so a read in
      // the first minute after a write can still get the previous body — see
      // the cache priming below, which is what closes that window.
  ...
  // Serve what we just wrote for at least as long as the CDN could hold the
  // old body. Without this the page regenerated by revalidatePath could read
  // the stale blob and cache that for another full revalidate period, so a
  // saved edit would look like it had not been saved at all.
  blobIndex = null;
  blobIndexExpires = 0;
  docCache.set(name, { value, expires: Date.now() + BLOB_CACHE_SECONDS * 1000 });

  return value;
}

/**
 * Which documents have been saved through the admin. Anything absent is still
 * being served from the copy bundled with the deployment, which also means a
 * later change to that file in the repository will still reach the site.
 */
export async function listStoredDocs() {
  if (!hasBlobStore()) return null;
  try {
    const index = await getBlobIndex();
    return [...index.keys()]
      .map((p) => p.slice(BLOB_PREFIX.length).replace(/\.json$/, ''))
      .sort();
  } catch (err) {
    console.error('[store] listStoredDocs failed:', err.message);
    return null;
  }
}
