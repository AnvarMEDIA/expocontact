...
import {
  ...
  isDate, daysBetween, addDays, roundMoney, STAGE_KEYS, isClosed, url,
  PAYMENT_KEYS, formatMoney,
} from './model.js';
 ...
 *
 * The payment carries its own `method`: a contract can say «перечисление»
 * while the deposit came in cash, and the cashbook has to show where the money
 * really landed. It falls back to the project's method when not given.
 * `receivedBy` is who took it — the question that comes up first when a sum
 * does not match the bank statement.
 */
export async function addPayment(id, { amount, date, note, method, receivedBy } = {}) {
  const p = await getProject(id);
  if (!p) return null;
  ...
  const payment = {
    id: taskId(),
    amount: value,
    currency: p.money?.currency || 'UZS',
    date: isDate(date) ? date : todayISO(),
    method: PAYMENT_KEYS.includes(method) ? method : (p.money?.method || ''),
    receivedBy: String(receivedBy || '').trim().slice(0, 80),
  ...
  return saveProject({
    ...
    history: [...(p.history || []), {
      ts: now(), type: 'payment',
      text: `Приход ${formatMoney(value, payment.currency)}${payment.receivedBy ? ` — принял ${payment.receivedBy}` : ''}`,
    }],
  });
}
...
/** Quotes, contracts, drawings. `kind` marks which of them is the quote. */
const FILE_KINDS = ['quote', 'contract', 'drawing', 'other'];

export async function addFile(id, { name, url: href, kind, size, ext } = {}) {
  const p = await getProject(id);
  if (!p) return null;
  ...
  const file = {
    id: taskId(),
    name: String(name || 'файл').trim().slice(0, 160),
    url: safeUrl,
    kind: FILE_KINDS.includes(kind) ? kind : 'other',
    size: Number(size) > 0 ? Number(size) : 0,
    ext: String(ext || '').slice(0, 8),
    ts: now(),
  };
  return saveProject({
    ...p,
    files: [...(p.files || []), file],
    history: [...(p.history || []), { ts: now(), type: 'file', text: `Файл: ${file.name}` }],
  });
}
...
export async function deleteFile(id, fileId) {
  ...
  if (files.length === (p.files || []).length) return null;
  // The blob itself is left in place: the same file may be linked from
  // elsewhere, and an orphan costs storage, while a wrong delete costs a quote.
  return saveProject({ ...p, files });
}
...
/** How far back a cashbook view looks. Keys are what the UI sends. */
export const CASH_RANGES = {
  today: 0, week: 6, month: 29, quarter: 89, year: 364, all: null,
};

/**
 * Every payment across every project, newest first, with totals.
 *
 * Currencies are kept apart at every level: adding sums to dollars would
 * produce a number that means nothing. Older payments predate the per-payment
 * currency field, so the project's currency is used as the fallback.
 */
export async function cashbook(range = 'month', today = todayISO()) {
  const days = range in CASH_RANGES ? CASH_RANGES[range] : CASH_RANGES.month;
  ...
  for (const p of docs) {
    for (const pay of p.payments || []) {
      const date = isDate(pay.date) ? pay.date : null;
      ...
      rows.push({
        ...
        currency: pay.currency || p.money?.currency || 'UZS',
        method: pay.method || p.money?.method || '',
  ...
  const totals = {};      // { UZS: 12345 } — received in the period
  const byMethod = {};    // { UZS: { cash: 500, transfer: 900 } }
  for (const r of rows) {
    totals[r.currency] = roundMoney((totals[r.currency] || 0) + r.amount, r.currency);
    byMethod[r.currency] = byMethod[r.currency] || {};
    const key = r.method || 'unknown';
    byMethod[r.currency][key] = roundMoney((byMethod[r.currency][key] || 0) + r.amount, r.currency);
  }

  // The whole cash position, ignoring the period: everything ever received,
  // and what is still owed on projects that are not closed.
  const allTime = {};
  const owed = {};
  for (const p of docs) {
    const cur = p.money?.currency || 'UZS';
    for (const pay of p.payments || []) {
      const c = pay.currency || cur;
      allTime[c] = roundMoney((allTime[c] || 0) + (Number(pay.amount) || 0), c);
    }
    if (isClosed(p.stage)) continue;
    const total = Number(p.money?.total) || 0;
    const paid = (p.payments || []).reduce((n, x) => n + (Number(x.amount) || 0), 0);
    if (total > paid) owed[cur] = roundMoney((owed[cur] || 0) + (total - paid), cur);
  }

  return {
    range, from, today,
    totals, byMethod, allTime, owed,
    count: rows.length,
    payments: rows.slice(0, 300),
  };
}
