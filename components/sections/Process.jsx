...
function StepCard({ step, index, total, isVisible, fullWidth = false }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 50 }}
      animate={isVisible ? { opacity: 1, y: 0 } : {}}
      transition={{ delay: index * 0.1, duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
      className={`relative flex-shrink-0 ${fullWidth ? 'w-full' : 'w-60 sm:w-64 md:w-72'}`}
    >
      {/* Connector arrow → (desktop horizontal only) */}
      {!fullWidth && index < total - 1 && (
      ...
      {/* Card */}
      <div
        className="group h-full p-5 sm:p-6 rounded-2xl bg-charcoal/70 border border-white/5 hover:border-gold/25 transition-all duration-500 hover:-translate-y-1"
      ...
      >
        ...
        <div className="relative mb-5 sm:mb-6">
          ...
          {/* Gold icon circle */}
          <div className="relative w-11 h-11 sm:w-12 sm:h-12 rounded-xl glass-gold flex items-center justify-center">
        ...
        <h3 className="font-heading font-black text-sm sm:text-base md:text-lg mb-2 text-white">{step.title}</h3>
...
export default function Process() {
  ...
  return (
    <section id="process" ref={sectionRef} className="py-16 sm:py-24 md:py-32 bg-navy overflow-hidden relative">
      {/* Horizontal gradient fade masks (desktop) */}
      <div className="absolute inset-y-0 left-0 w-20 md:w-32 bg-gradient-to-r from-navy to-transparent z-10 pointer-events-none hidden md:block" />
      <div className="absolute inset-y-0 right-0 w-20 md:w-32 bg-gradient-to-l from-navy to-transparent z-10 pointer-events-none hidden md:block" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="font-heading font-black text-2xl sm:text-3xl md:text-5xl leading-tight heading-accent">
            {t('title')}
          </h2>
          <p className="text-white/40 text-base sm:text-lg mt-4 sm:mt-5">{t('subtitle')}</p>
        </motion.div>
      </div>
      ...
      <div className="md:hidden max-w-7xl mx-auto px-4 sm:px-6">
        <div className="relative">
          ...
          <div className="absolute left-[1.375rem] top-14 bottom-14 w-px bg-gradient-to-b from-gold/30 via-gold/10 to-transparent" />
