...
// Placeholder gradient colors per index (used when no image is set)
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        {/* Panel */}
        <motion.div
          className="relative bg-charcoal border border-white/10 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl"
        ...
        >
          {/* Image */}
          <div className="relative aspect-[16/9] bg-navy rounded-t-2xl overflow-hidden">
          ...
          <div className="p-6 md:p-8">
            <h3 className="font-heading font-black text-2xl mb-2">{project.title}</h3>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-white/10">
              ...
              <button
                onClick={onClose}
                className="px-5 py-2 bg-white/5 text-white/60 rounded-lg hover:bg-white/10 transition-colors text-sm"
              >
                {t('close')}
              </button>
            </div>
...
function PortfolioCard({ project, index, t, onClick }) {
  ...
  return (
    <motion.article
      ...
      onClick={onClick}
      className="group relative cursor-pointer rounded-xl overflow-hidden border border-white/5 hover:border-gold/40 transition-all duration-300 bg-charcoal"
      style={{ boxShadow: '0 0 0 0 rgba(212,168,67,0)' }}
    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden">
        {hasImage ? (
          ...
          <div className={`absolute inset-0 bg-gradient-to-br ${color} flex items-center justify-center`}>
            {/* Architecture-themed placeholder icon */}
            <svg className="w-14 h-14 text-gold/20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={0.75}
                d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z" />
              <polyline strokeLinecap="round" strokeLinejoin="round" strokeWidth={0.75}
      ...
      {/* Card body */}
      <div className="p-4">
        <h3 className="font-heading font-black text-sm leading-snug mb-1 text-white group-hover:text-gold-light transition-colors">
          {project.client}
        </h3>
  ...
  return (
    <section id="portfolio" className="py-24 md:py-32 bg-charcoal">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {filtered.length === 0 && (
          <p className="text-center text-white/30 py-16">Нет проектов в этой категории</p>
        )}
      </div>

      {/* Lightbox */}
      {selected && (
        <Lightbox project={selected} onClose={() => setSelected(null)} t={t} />
      )}
    </section>
  );
}
